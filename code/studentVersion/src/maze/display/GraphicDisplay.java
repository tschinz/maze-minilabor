...
public class GraphicDisplay {
	...
	public class Display extends ImageGraphicsMultiBuffer {
		...
		/**
		 * Sets the text that is displayed at the bottom of the screen
		 * 
		 * @param msg
		 */
		...
		public void render(Graphics2D g) {
			 ...
			 * decoration
			 */
			int border_top = this.mainFrame.getInsets().top;
			int border_left = this.mainFrame.getInsets().left;
			...
			for (int i = 0; i < nCellsX; i++) {
				...
				for (int j = 0; j < nCellsY; j++) {
					...
					if (e.isExit) {
						...
						g.fillRect(
								xs + (int) Math.round(strokeSize / 2.0), ys + (int) Math.round(strokeSize / 2.0),
								wCell, hCell);
						g.setColor(Color.black);
					}
			...
			/**
			 * Draw HES-SO logo, centered, at the bottom of the screen
			 */
			g.drawImage(mBitmap, fWidth / 2 - mBitmap.getWidth() / 2, fHeight - 30, null);

			// Write some information message
			if (msg != null)
				g.drawString(msg, 5, fHeight - 40);
		}
	}
	...
	public GraphicDisplay(MazeContainer mc, int sizeOfSquare, boolean decorations) {
		 ...
		 * strokeWidth)
		 */
		frameWidth = (nCellsX * wCell + ((nCellsX + 1) * strokeSize));
		frameHeight = (nCellsY * hCell + ((nCellsY + 1) * strokeSize));

		// Load the image
		loadImage("/images/logo_hei.png");

		// Create a display and keep some space for the picture and the text at
		// the bottom
		disp = new Display("Maze - Minilabor", frameWidth, frameHeight + 55, decorations);
	...
	public static void main(String args[]) {
		...
		// Display the maze
		GraphicDisplay gd = new GraphicDisplay(mc, 10, false);
	}
}
